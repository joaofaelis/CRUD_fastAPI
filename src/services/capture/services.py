...
class Capture(Service_Pokemon):
    ...
    def capture_pokemon(cls, collection_capture,collection_pokemon, name_pokemon, unique_id, number_pokemon):
        ...
        get_info: dict = Repository.get_object(collection_pokemon, object_find)
        if get_info is None:
            ...
            Repository.insert_object(collection_capture, pokemon_objects)
            ...
            Repository.insert_object(collection_capture, get_info)
            return get_info
    ...
    def get_captures(cls, collection, unique_id):
        ...
        capture_find = Repository.get_object(collection, query)
        obj_captured = {
    ...
    @classmethod
    def pagination_cap(cls, collection, skip, limit):
        collectio = Repository.get_collection(collection)
        skip_limit = (collectio.find({"status": True}).skip(skip).limit(limit))
    ...
    def update_name(cls, unique_id, name_pokemon):
        ...
        give_obj = Repository.get_object('capture', update_obj)
        up_date = {
            "name_pokemon": name_pokemon,
        }
        execute_up = Repository.update_object('capture', give_obj, {"$set": up_date})
        return execute_up
    ...
    def delete_cap(cls, unique_id):
        ...
        delete = Repository.delete_object('capture', select)
        return delete
